...
### D6. Именованный доступ и TLS: `b2b-crm.local`, самоподписанный CA

Сайт доступен только по HTTPS на `https://b2b-crm.local` (HTTP не
поднимается, стандартный порт 443). Требуется: запись `127.0.0.1
b2b-crm.local` в `/etc/hosts` хоста (Linux; `sudo nano /etc/hosts` или
команда в README) и самоподписанные сертификаты.
...
`b2b-crm.local, localhost, 127.0.0.1, host.docker.internal`, сроком 10 лет
...
сертификат. Уже существующий volume `/certs` со старым SAN не
перегенерируется скриптом — при смене имени хоста его нужно удалить
(`docker compose down -v` или удаление volume `certs`).
...
- **Почему 443, а не 8443:** стандартный порт HTTPS — адрес в браузере
  без явного порта; порт всё равно выносится в `.env` (`HTTPS_PORT`) и
  при конфликте на хосте может быть переопределён.
- **Почему `b2b-crm.local`, а не `b2b-crm.loc`:** пользователь выбрал
  `.local`-имя для локальной разработки; для браузеров с mDNS-резолвером
  (`*.local`) приоритет остаётся за `/etc/hosts`.
