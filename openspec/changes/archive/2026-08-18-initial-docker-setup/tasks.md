...
- [x] 2.4 Убедиться, что приложение отвечает через `nginx` (health-роут, страница без ошибок)
...
- [x] 4.5 Проверить оба способа запуска (`make e2e` и из OpenCode), повторный запуск миграций/fixtures идемпотентен
